...
public class MyIndentFragment extends BaseFragment implements OnRefreshListener, OnLoadMoreListener {
        ...
        bmobQuery.findObjects(new FindListener<Post>() {
            ...
            public void done(List<Post> queryList, BmobException e) {
                if (e == null) {
                    ...
                    //添加请求的数据
                    MyIndentFragment.this.list.addAll(queryList);
                    adapter.updateList(MyIndentFragment.this.list);
                    isShowEntryViweLayout(MyIndentFragment.this.list);
                } else {

                    Log.i(TAG, "done: 请求失败====》 "+ e.getMessage() + "," + e.getErrorCode());
...
//                    showToast("刷新失败！" + e.getMessage() + "," + e.getErrorCode());
    ...
    private void addQueryParmas(BmobQuery<Post> bmobQuery, int skip) {
        ...
        bmobQuery.addWhereNotEqualTo(Post.STATE, true);
