...
public class StringUtil {
     ...
     * 解析字符串，解析成数组返回
     */
    public static ArrayList<String> parseArrayForString(String arrayStr) {
     ...
     * 通过逗号解析地理位置经纬度的字符串数组
     */
    public static String[] getMapLatLngArray(String navLatLng) {
        if (navLatLng == null || !navLatLng.contains(",") || navLatLng.isEmpty()) {
            return null;
        }
        String[] mapArray = navLatLng.split(",");
    ...
    //根据需要的值查询下标，查不到则返回-1
    public static int queryIndexForContent(String content, String[] array) {
        if (content == null || array == null) {
            return 0;
        }
